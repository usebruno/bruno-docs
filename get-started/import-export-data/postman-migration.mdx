...
![bulk import](/images/screenshots/migration/bulk-import.webp)
...
If your Postman account has grown large or disorganized, the [Postman Export Organizer](/get-started/import-export-data/postman-export-organizer) can group your data dump by workspace, flag duplicates, and produce a ZIP ready for bulk import.
...
![Install packages dialog](/images/screenshots/v4/chores/install-packages-dialog.webp)
...
![Install packages dialog](/images/screenshots/v4/chores/install-package-developer-mode.webp)
...
If an imported name already exists in the collection, Bruno prompts you to **Replace**, import as **New** (a copy), or **Skip**. See [Duplicate environment names](/get-started/import-export-data/import-export-environments#duplicate-environment-names). A Postman data dump that contains environments with the same name is imported without that prompt, Bruno handles those names internally.
...
## Preserve Postman Scripts

By default, Bruno translates Postman script APIs (such as `pm.test` and `pm.environment`) to their Bruno equivalents during import and export. 
If you have complex scripting setups and want Bruno to leave scripts untouched, enable **Preserve scripts**. When this option is on:

- Scripts are kept **verbatim** as `pm.*` during import.
- Collection structure, requests, variables, and settings are still converted as usual during import.
- Scripts are converted to Bruno script APIs during export.

### Enable Preserve scripts on import

1. Click the **+** icon in the top-left sidebar and select **Import Collection**.
2. Choose your Postman collection file and continue to the import dialog.
3. Click **Options**, then expand **Show Advanced Options**.
4. Turn on the **Preserve scripts** toggle (off by default).
5. Click **Import**.

![Preserve scripts option](/images/screenshots/v4/chores/preserve-postman-scripts.webp)

The same **Preserve scripts** option is available when exporting a Bruno collection to Postman format, so you can round-trip collections without Bruno rewriting your scripts.
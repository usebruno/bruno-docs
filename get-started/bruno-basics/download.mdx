...
description: "Install Bruno on macOS (DMG, PKG installer, Homebrew), Windows (EXE, MSI, winget, Chocolatey, Scoop) and Linux (AppImage, deb, rpm, APT, Flatpak, Snap)."
---
...
<Tabs>
  <Tab title="macOS">
    ...
    <Info>
      **PKG Installer**
      ...
      Starting with v4.0.0, Bruno is also available as a `.pkg` installer for MDM tools and scripted deployments.
      
      - Installs `Bruno.app` to `/Applications` for all users and requires administrator privileges
      - Signed and notarized by Apple. Verify with `pkgutil --check-signature <file>.pkg`
      - Package identifier: `com.usebruno.app`
      
      Example: `sudo installer -pkg bruno_<version>_<arch>_mac.pkg -target /`
    </Info>
  ...
  <Tab title="Windows">
    ...
    Bruno runs on **Windows 10** and **Windows 11**. 
    ### Direct Download
...
<Tabs>
  <Tab title="Bruno App">
    ...
    |----------|-----------------|
    | macOS (Apple Silicon) | `.dmg` or `.pkg` (arm64) |
    | macOS (Intel) | `.dmg` or `.pkg` (x64) |

...
![Advanced options – choose location](/images/screenshots/get-started/bruno-basics/create-workspace/advanced.option.webp)
...
If you opened a collection from a location outside the active workspace folder, you can move it into the workspace so it is managed alongside the rest of your collections.

<Info>
  The **Move into Workspace** option only appears for collections whose files live outside the workspace root. Collections already inside the workspace do not show this option.
</Info>

1. In the sidebar, hover over the collection and click the **three-dot menu**.
2. Select **Move into Workspace**.
3. Review the source path and the destination (`<workspace>/collections/<name>`), then click **Move**.

Bruno moves the collection files to `<workspace>/collections/<name>`, registers the new path in `workspace.yml`, and re-opens the collection from its new location. Any open request tabs for that collection will close.

**Example**: The collection `legacy-proxy` was opened from outside the active workspace. After you move it, Bruno copies the files under `collections/` and updates the path in `workspace.yml`.

<CodeGroup>

```yaml Before (workspace.yml)
...
collections:
  - name: "users-api"
    path: "collections/users-api"
  - name: "payments-api"
    path: "collections/payments-api"
  - name: "legacy-proxy"
    path: "../../other-projects/legacy-proxy"
```

```yaml After (workspace.yml)
opencollection: 1.0.0
info:
  name: "api-workspace"
  type: workspace
...
collections:
  ...
  - name: "payments-api"
    path: "collections/payments-api"
  - name: "legacy-proxy"
    path: "collections/legacy-proxy"
```

</CodeGroup>

<Warning>
  If the collection has unsaved changes, Bruno shows a confirmation dialog. You can **Save All and Move**, **Discard All and Move**, or save individual unsaved requests before continuing.
</Warning>

...
- Remove any attribute mappings not listed above to prevent provisioning errors — **except** the `roles` mapping described in [Configure Admin Role Mapping](#configure-admin-role-mapping) below, if you're using that feature
</Warning>
...
![Create app role in Entra ID](/images/screenshots/sso-scim-management/entra/entra-scim-9.webp)
...
1. Under your Enterprise Application's **Provisioning** → **Mappings** → **Provision Microsoft Entra ID Users**, click **Add New Mapping**
2. Configure:
...
![Configure roles attribute mapping](/images/screenshots/sso-scim-management/entra/entra-scim-10.webp)
...
![Configure Admin Role Mapping in Bruno](/images/screenshots/sso-scim-management/entra/entra-scim-11.webp)
...
3. Select the users or groups you want to provision with Bruno licenses. If you [configured Admin Role Mapping](#configure-admin-role-mapping) and want a user provisioned as an admin, select that App Role under **Select a role** instead of the default access role
...
The App Role assignment was not in place when Entra ID first provisioned the user. Assign the App Role before the user is provisioned, then use [on-demand provisioning](https://learn.microsoft.com/entra/identity/app-provisioning/provision-on-demand) to confirm the `roles` value appears in the request.

To correct an affected user, remove their license under **Users** in the Bruno portal. Re-provisioning through Entra ID does not remove an existing license.

...
| bru.setCookie(url, name, value) | Sets a single cookie using key-value format.              |
| bru.setCookie(url, cookieObject) | Sets a single cookie using object format.                 |
| bru.setCookies(url, cookies)     | Sets multiple cookies at once using an array of cookie objects. |
| bru.getCookie(url, name)         | Gets a specific cookie by name.                           |
| bru.getCookies(url)              | Gets all cookies for a specific URL.                      |
| bru.deleteCookie(url, name)      | Deletes a specific cookie by name.                        |
| bru.deleteCookies(url)           | Deletes all cookies for a specific URL.                   |
| bru.jar.clear()                  | Clears all cookies from the cookie jar.                   |
...
bru.setCookie(url, name, value);

// Overload 2: Object format
bru.setCookie(url, cookieObject);
```
...
bru.setCookie("https://example.com", "sessionId", "abc123");

// Set a cookie with options using object format
bru.setCookie("https://example.com", {
  key: "userToken",
  value: "xyz789",
  domain: "example.com",
  path: "/api",
...
**Example:**
```javascript
bru.setCookies("https://example.com", [
  {
    key: "sessionId",
    value: "abc123",
    secure: true,
...
**Example:**
```javascript
const sessionCookie = bru.getCookie("https://example.com", "sessionId");
if (sessionCookie) {
...
**Example:**
```javascript
const allCookies = bru.getCookies("https://example.com");
...
```javascript
// Delete a cookie
bru.deleteCookie("https://example.com", "sessionId");
```
...
```javascript
// Delete all cookies for a specific URL
bru.deleteCookies("https://example.com");
```

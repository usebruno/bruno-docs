...
| req.getBody()                  | Get the current request body/payload.                                |
...
Get the current request tags as an array of strings. This method allows you to access the tags associated with the current request, which can be useful for conditional logic, filtering, or organizing requests based on their tags.
...
Set the Bruno environment variable. By default, environment variables are temporary and do not persist between app restarts.
...
```javascript
// In post-request script of "one.bru"
...
```javascript
// If you have: folder1/request-name.bru
// Use just the request name, not the full path

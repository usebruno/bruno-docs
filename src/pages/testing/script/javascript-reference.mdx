...
| bru.setCookie(name, value, options) | Sets a single cookie with specified attributes.        |
| bru.setCookies(cookies)          | Sets multiple cookies at once using an array of cookie objects. |
| bru.getCookie(name)              | Gets a specific cookie by name.                           |
| bru.deleteCookie(name, options)  | Deletes a specific cookie by name.                        |
| bru.deleteCookies(names, options) | Deletes multiple cookies by name or all cookies.         |
...
Bruno provides Cookie Jar APIs that enable programmatic cookie management in request scripts. These APIs allow you to set, get, and delete cookies programmatically.
...
Set a single cookie with specified attributes.

**Syntax:**
```javascript
bru.setCookie(name, value, options);
```

**Parameters:**
- `name`: The name of the cookie
- `value`: The value of the cookie
- `options`: (Optional) Cookie options object with the following properties:
...
```javascript
// Set a simple cookie
bru.setCookie("sessionId", "abc123");

// Set a cookie with options
bru.setCookie("userToken", "xyz789", {
  domain: "example.com",
  path: "/api",
  secure: true,
  httpOnly: true,
...
**Example:**
```javascript
const sessionCookie = bru.getCookie("sessionId");
if (sessionCookie) {
...
**Syntax:**
```javascript
bru.deleteCookie(name, options);
```

**Parameters:**
- `name`: The name of the cookie to delete
- `options`: (Optional) Cookie options object (domain, path) that must match the original cookie

**Example:**
```javascript
// Delete a cookie
bru.deleteCookie("sessionId");

// Delete a cookie with specific domain and path
bru.deleteCookie("userToken", {
  domain: "example.com",
  path: "/api"
});
```
...
Delete multiple cookies by name or delete all cookies.

**Syntax:**
```javascript
bru.deleteCookies(names, options);
```

**Parameters:**
- `names`: Array of cookie names to delete, or `null` to delete all cookies
- `options`: (Optional) Cookie options object (domain, path) that must match the original cookies

**Example:**
```javascript
// Delete specific cookies
bru.deleteCookies(["sessionId", "userToken"]);

// Delete all cookies
bru.deleteCookies(null);

// Delete cookies with specific domain and path
bru.deleteCookies(["sessionId", "userToken"], {
  domain: "example.com",
  path: "/api"
});

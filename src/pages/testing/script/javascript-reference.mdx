...
| jar.setCookie(url, name, value) | Sets a single cookie using key-value format.              |
| jar.setCookie(url, cookieObject) | Sets a single cookie using object format.                 |
| jar.setCookies(url, cookies)     | Sets multiple cookies at once using an array of cookie objects. |
| jar.getCookie(url, name)         | Gets a specific cookie by name.                           |
| jar.getCookies(url)              | Gets all cookies for a specific URL.                      |
| jar.deleteCookie(url, name)      | Deletes a specific cookie by name.                        |
| jar.deleteCookies(url)           | Deletes all cookies for a specific URL.                   |
| jar.clear()                      | Clears all cookies from the cookie jar.                   |

...
#### `stopExecution`
You can terminate the collection run by using `bru.runner.stopExecution()` in the pre-request, post-request, or the test scripts. This function is effective only within the context of a collection run.

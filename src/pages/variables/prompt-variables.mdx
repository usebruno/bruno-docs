...
- The text after the question mark (`?`) is the prompt message shown to the user.
...
<Callout type="info">
  If you leave a prompt variable blank, it will be replaced with an empty string
  in the request.
</Callout>
...
<Callout type="warning">
  Prompt variables are only available during the execution of a request and are
  not persisted or accessible after the request completes.
</Callout>

...
## Behavior in CLI and Runners

<Callout type="warning">
  Prompt variables are only available when executing requests interactively in the Bruno. They are **not supported** in automated execution environments.
</Callout>

### Bruno CLI

When running collections via Bruno CLI, any requests containing prompt variables will be **skipped** and marked as `skipped` in the CLI output. This is because CLI does not support interactive prompting.

### Collection Runner

Similarly, when running collections using the Bruno Runner, requests containing prompt variables will be **skipped** and marked as `skipped` in the run results.

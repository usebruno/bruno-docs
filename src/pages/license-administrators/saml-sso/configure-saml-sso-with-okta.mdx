...
Before configuring a SAML application in Okta, you must configure SSO in Bruno.
...
3. In the Bruno SSO settings page, you'll see the **SP Issuer ID / Entity ID** field.
   - Set your own unique identifier (e.g., `bruno-sso`, `your-company-bruno`, etc.)

   Note the following values (you'll need these when configuring Okta):
...
**Important**: Always copy the SAML ACS URL directly from the Bruno SSO settings page. The URL format may differ from the documentation depending on your Bruno version.
</Callout>
...
1. In the **Single sign-on URL** field, paste the **SAML ACS URL** from Bruno
   - Example:`https://license.usebruno.com/api/v2/auth/sso/saml/acs/XXXXXXXXXXXXXXXXXXXXXXXX`
   - Use the exact URL shown in your Bruno SSO Configuration page
...
1. In the **Audience URI (SP Entity ID)** field, paste the **SP Issuer ID / Entity ID** value from Bruno
...
{/* TODO: Add screenshot - okta-saml-settings.png
     Shows the complete SAML settings section
     Alt text: "Complete SAML settings configuration in Okta" */}
...
2. **Map to existing Okta user attribute**: If your Okta users already have a role attribute
   - Example: Value = `user.userType` or `user.role`
...
<Image src="/screenshots/sso-scim-management/okta/okta-attribute-statements.webp" alt="Configure attribute statements in Okta showing roles and fullName" width={800} height={600} className="rounded mt-4" />
...
### Step 7: Copy SAML Metadata

1. In your Bruno app, navigate to the **Sign On** tab
...
3. Copy the following values (you'll need these for Bruno configuration):
   ...
   - **Issuer**: Copy this value
   - **Certificate**: Download or copy the X.509 certificate

{/* TODO: Add screenshot - okta-saml-metadata.png
     Shows the SAML 2.0 section with metadata details
     Alt text: "Retrieve SAML metadata from Okta" */}

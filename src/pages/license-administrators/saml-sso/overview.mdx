...
- **Single set of credentials**: Users access Bruno with the same credentials they use for other corporate applications
- **Reduced password fatigue**: No need to remember separate passwords for Bruno
...
- **Simplified onboarding**: New employees automatically receive Bruno access as part of their standard onboarding
- **Simplified offboarding**: Departing employees automatically lose Bruno access when removed from your IdP
...
- **Domain verification** completed in Bruno (if required)
...
- Enable SAML SSO
- Enter your IdP's SAML metadata (Login URL, Certificate, Entity ID)
- Configure role mappings to match your IdP's role attributes
...
| Field | Value | Notes |
|-------|-------|-------|
| **ACS URL** | Varies by version | Copy the exact SAML ACS URL from your Bruno SSO settings page |
| **Entity ID (SP Issuer ID)** | Your choice | Can be any unique identifier (e.g., `bruno`, `bruno-demo`). Must match exactly between Bruno and your IdP |
| **Name ID Format** | Email Address | Required for user identification |

<Callout type="info">
**Important**: Always copy the ACS URL and Entity ID directly from your Bruno SSO settings page. The Entity ID can be any unique value you choose, but it must match exactly between Bruno and your identity provider.
</Callout>
...
| Attribute Name | Name Format | Description | Example Value |
|----------------|-------------|-------------|---------------|
| **roles** | Unspecified | User role(s) for access control | Any role value from your IdP (e.g., `BrunoAdmin`, `Engineering`, `admin`, `user`) |
| **fullName** | Unspecified | User's full name | `user.firstName+" "+user.lastName` or equivalent |
...
3. If it matches "Admin Roles", the user gets admin access
4. If it matches "User Roles", the user gets standard access
5. If it doesn't match either, the user may be denied access

  ...
  return (
    <CommandDialog open={open} onOpenChange={setOpen} expanded={expanded}>
      <div className="flex items-center border-b-2 pl-3">
        ...
        >
          {expanded ? <Shrink height={16} width={16} /> : <Expand height={16} width={16} />}
        </Button>
      </div>
      ...
      <CommandList className={cn('p-2 h-full', expanded ? '' : 'h-full')}>
        {searchValue.length === 0 ? (
          Object.keys(history || {}).length > 0 ? (
            <div className={cn('flex flex-col w-full px-2 sm:px-3', expanded && 'px-2 sm:px-10')}>
              <History history={history} setOpen={setOpen} expanded={expanded} />
            </div>
          ) : (
...
export const CommandMenuTrigger = ({ className = '' }: { className?: string }) => {
  const [open, setOpen] = useState(false);
  useEffect(() => {
  ...
  return (
    <>
      ...
      />
      <Button
        className={cn(className, 'pr-1.5 h-8 sm:h-10 py-1 sm:py-2 px-2 sm:px-4')}
        variant="outline"

...
const FormattedSentence = ({
  ...
  return splitted ? (
    <Typography
      variant="muted"
      className={cn('max-sm:text-xs mt-1 sm:mt-2 line-clamp-1 truncate', className)}
    >
      {transformed}
      ...
      >
        <Link href={`/${result.path}`}>
          ...
          <div className="flex flex-col mx-4 items-start justify-center overflow-hidden grow">
            <Badge variant="secondary" className="bg-zinc-200 dark:bg-zinc-700 capitalize">
              {namePath}
            </Badge>
            {sentence && <FormattedSentence sentence={sentence} keyword={keyword} />}
          </div>

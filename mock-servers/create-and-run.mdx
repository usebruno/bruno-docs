...
A rule matches on a request header (case-insensitive), query parameter, or body field, using equals, not equals, contains, or matches (regex). Body fields use a dotted path. For example, `$.user.type` or `user.type`. Array indexing and JSONPath filters are not supported.
...
Example: if a Header rule requires **x-api-key** to equal **1234567890**, the demo request includes **x-api-key: 1234567890** as a request header.  

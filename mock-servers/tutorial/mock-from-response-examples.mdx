...
<Warning>
  Mock Servers are in **Beta**. They are on by default and appear in the sidebar. To hide them, turn off **Mock Server** under **Preferences → Beta**.
</Warning>
...
2. In the Response tab, click the **Save Response** *(Bookmark)* icon in the top-right corner.
...
Bruno opens the mock [dashboard](/mock-servers/run-mock-server#dashboard-interface-walkthrough) with the **Responses** tab open.

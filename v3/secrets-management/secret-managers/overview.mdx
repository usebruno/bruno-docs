---
sidebarTitle: "Overview"
---

import { PremiumBadge } from "/snippets/premium-badge.jsx";

# Integration with Secret managers <PremiumBadge noLink />

For users of Bruno Ultimate, you can take your secret management one step further and perform an integration with a secret vault. 

<Tip icon="">
Bruno stores secrets on your local machine. The location depends on the OS. The secrets are encrypted using OS level encryption when available or falls back to AES256 encryption.
</Tip>

Currently, we support integrations with:

- [HashiCorp vault](/v3/secrets-management/secret-managers/hashicorp-vault/overview)
- [AWS Secrets Manager](/v3/secrets-management/secret-managers/aws-secrets-manager/overview)
- [Azure Key Vault](/v3/secrets-management/secret-managers/azure-key-vault/overview)

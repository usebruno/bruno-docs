...
An environment can inherit from **one** parent of the **same scope** (collection environments inherit collection environments; global environments inherit global environments). Chains are allowed: `STG-GW` → `STG` → `base`. The child's own values always win.

Inside the environment tier, Bruno merges **furthest ancestor → ancestors → direct parent → active environment**. Other Bruno scopes are unchanged: global, collection, resolved environment, folder, request, runtime, prompt, process.env, secrets.

...
- Inside an environment, values can [inherit from a parent](/variables/environment-variables#inherit-from-a-parent-environment). The merge is ancestor → parent → active environment, then the usual scope stack above still applies.

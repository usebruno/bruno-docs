...
<Warning>
  gRPC scripting is in **Beta**. Enable it under **Preferences → Beta → gRPC Scripting**. The Script tab on a gRPC request appears only after you opt in.
</Warning>
...
1. Enable **gRPC Scripting** under **Preferences → Beta**.
2. Open a gRPC request and go to the **Script** tab.
3. Choose a hook and write JavaScript. Autocomplete and lint cover `bru.grpc.*`.

...
3. Enter the **Key**, **Value**, **Type**, and an optional **Description** to document what the field represents.
5. Save and execute the request.

![multipart-data](/images/screenshots/v4/description/multipart.webp)
...
<Tip>
  You can use variable interpolation in both parameter values and file attachment paths. For example, setting a file value to a variable containing a filepath like `{{filepath}}` resolves the variable at runtime, letting you dynamically control which file is uploaded based on your environment or script variables.
</Tip>
...
3. Enter the **Key**, **Value**, and an optional **Description** to document what the field represents.
4. Save and execute the request.

![url-encoded](/images/screenshots/v4/description/unl_encoded.webp)
...
![raw-data](/images/screenshots/send-request/rest/raw-data-json.webp)
...
3. Two controls are available:
   - **File / Binary**: Send a file directly as the request body. Click **Select File** (or drag-and-drop) to pick the file, or enter a `{{variable}}` in the file path field to resolve the variable to the path dynamically at runtime. Bruno sends the file as-is (Content-Type will be inferred or set it in **Headers**).

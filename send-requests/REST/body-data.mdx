...
<Tip>
  You can use variable interpolation in both parameter values and file attachment paths. For example, setting a file value to `{{filePath}}` resolves the variable at runtime, letting you dynamically control which file is uploaded based on your environment or script variables.
</Tip>
...
3. Two controls are available:
   - **File / Binary**: Send a file directly as the request body. Click **Select File** (or drag-and-drop) to pick the file, or enter a `{{variable}}` in the file path field to resolve the path dynamically at runtime. Bruno sends the file as-is (Content-Type will be inferred or set it in **Headers**).

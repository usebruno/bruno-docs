...
3. Two controls are available:
   - **File / Binary**: Send a file directly as the request body. Click **Select File** (or drag-and-drop) to pick the file. Each row in the file table lets you set the file's **Content-Type**, mark it as **Selected** (the row Bruno sends when the request runs), and add an optional **Description** to document what the file is for. Bruno sends the selected file as-is (Content-Type will be inferred or set it in **Headers**).

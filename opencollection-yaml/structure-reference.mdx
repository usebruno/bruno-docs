...
For `multipart-form`, each entry in `data` can carry a per-part `contentType` that Bruno preserves on import, save, and export:
...
An empty or whitespace-only `contentType` is omitted from the exported file.

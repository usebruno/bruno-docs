...
| **Providers** | HashiCorp Vault only (CLI) | HashiCorp Vault, AWS Secrets Manager, Azure Key Vault, Google Cloud Secret Manager |
...
**The `--secrets-env-file` flag is required for AWS, Azure, and GCP in CI.**
Unlike HashiCorp Vault (which can use `--env-var` for token auth), AWS, Azure, and Google Cloud credentials must be passed via `--secrets-env-file`. Flat `--env-var` flags are not read by those credential resolvers.
...
- [Bruno v4 release notes](https://www.usebruno.com/v4-release)

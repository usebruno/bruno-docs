...
![dot env vars](/images/screenshots/v4/secret-variables/process-env.webp)
...
If you manage `.env` variables through Bruno's UI, values containing special characters are automatically quoted when saved. However, If you edit the file manually, wrap values in double quotes when they contain any of the following:

| Character | Why it needs quoting |
|---|---|
| `#` | The dotenv parser treats everything after `#` as a comment |
| `\n` / `\r` | Newlines break the line-based format |
| `"` | Must be escaped as `\"` inside a quoted value |
| `\` | Must be escaped as `\\` inside a quoted value |
...
<Warning>
  Example: `PASSWORD=ABC#DEF` is parsed as `ABC`, not `ABC#DEF`. Always quote values that contain `#`.
</Warning>

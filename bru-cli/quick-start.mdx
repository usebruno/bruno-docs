...
Use `--env-var` for **collection-level** variables:

```bash
bru run --env local --env-var baseURL=https://echo.usebruno.com
```

You can chain multiple collection overrides:
...
Use `--global-env-var` for **global environment** variables. This requires `--global-env`, and is the correct flag when scripts call `bru.getGlobalEnvVar()` or when you need to inject global secrets in CI:
...
You can combine both scopes in one run:
...
<Tip>
  Secret variables are not persisted to disk for CLI use. Pass collection secrets with `--env-var`, and global secrets with `--global-env-var`.
</Tip>

**Success Criteria:**
- Collection runs successfully with overridden variables.
- `bru.getGlobalEnvVar()` returns values passed via `--global-env-var`.

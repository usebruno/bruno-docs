...
If you are seeing unbounded RSS(Resident Set Size) growth or OOM(Out of Memory) kills, you can use the `--experimental-cache-modules` flag to reduce the memory usage. It will share evaluated script modules across requests in a `bru run`.

If you are **not** seeing unbounded RSS growth or OOM kills, leave the default as-is.

### `--experimental-cache-modules`

Pass this flag on `bru run` when a run grows memory with every request:

```bash copy
bru run --experimental-cache-modules
```
...
```yaml
- run: bru run --env ci --experimental-cache-modules --reporter-junit results.xml
```
...
`--exclude-envs` takes preference. If the same environment name appears in both `--envs` and `--exclude-envs`, that environment is left out.

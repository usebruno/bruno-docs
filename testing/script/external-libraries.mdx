...
You can also read a CSV file. Enable [Developer Mode](/get-started/configure/javascript-sandbox) so `fs` and `path` are available.
...
`__dirname` is the directory of the executing file (the request, folder, or collection script). `bru.cwd()` is always the collection root. See [`__dirname` and `__filename`](/testing/script/javascript-reference#__dirname-and-__filename).

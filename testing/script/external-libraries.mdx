...
<Note>Starting with Bruno v3.1.0, YAML (OpenCollection format) is the default file format when creating new collections. Learn more about create collection [here](/get-started/bruno-basics/create-a-collection).</Note>
    ...
    <Tree>
      <Tree.Folder name="bruno-collection" defaultOpen>
        <Tree.Folder name="external-lib" defaultOpen>
          <Tree.File name="echo-api.yml" />
          <Tree.File name="auth.yml" />
        </Tree.Folder>
...
You can also read CSV from a file on disk. Enable [Developer Mode](/get-started/configure/javascript-sandbox) so `fs` and `path` are available.

Place `data.csv` **next to the request** `.yml` file and use `__dirname`:
...
If the file lives at the **collection root** (or in a shared folder under it), use `bru.cwd()` instead:

```javascript
const csvData = fs.readFileSync(path.join(bru.cwd(), 'data.csv'), 'utf8');
```

`__dirname` is the directory of the executing `.bru` file (the request, folder, or collection script). `bru.cwd()` is always the collection root. See [`__dirname` and `__filename`](/testing/script/javascript-reference#__dirname-and-__filename).

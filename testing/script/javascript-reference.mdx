...
| bru.utils.minifyJson(json)       | Minifies a JSON string or object.                         |
| bru.utils.minifyXml(xml)         | Minifies an XML string.                                   |
...
The `bru.utils` namespace provides helper methods for formatting and minifying data.

- **`utils.minifyJson(json)`** - Minifies a JSON string or object by removing all unnecessary whitespace.

  **Parameters:**
  - `json` (string or object): The JSON data to minify. Accepts either a parsed object or a JSON string.
  ...
  **Examples:**
  ...
  const minified = bru.utils.minifyJson({ name: "Bruno", version: "1.0" });
  console.log(minified); // '{"name":"Bruno","version":"1.0"}'
  ...
  <Warning>
    Throws an error if the input is `null`, `undefined`, or an invalid JSON string. Only accepts strings and objects.
  </Warning>

- **`utils.minifyXml(xml)`** - Minifies an XML string by removing indentation and line breaks.

  **Parameters:**
  - `xml` (string): The XML string to minify.

  **Returns:** A minified XML string.
  ...
  **Example:**
  ...
  `;
  const minified = bru.utils.minifyXml(xml);
  console.log(minified); // '<root><name>Bruno</name><version>1.0</version></root>'
  ```

  <Warning>
    Throws an error if the input is `null`, `undefined`, or not a string.
  </Warning>

...
<Note>
  This page covers the APIs available in **scripts and tests** (`req`, `res`, `bru`, and `bru.grpc` on gRPC requests). [Apps](/apps/overview) use a separate API — `bru.ctx` — which is injected into the App's page instead of the scripting sandbox, and is not available in scripts. See the [Apps API Reference](/apps/api-ref) for the full `bru.ctx` surface.
</Note>
...
<Warning>
  **v4 breaking change:** All write and delete APIs now persist to disk by default. The `{ persist: true }` option has been removed. Scripts that relied on non-persistent (in-memory only) behavior will behave differently after upgrading to v4.
</Warning>
...
| bru.setEnvVar(key, value)       | Sets an environment variable and persists it to disk.         |
| bru.deleteEnvVar(key)           | Deletes a specific environment variable and persists the change. |
...
| bru.getGlobalEnvVar(key)        | Get a global environment variable.                            |
| bru.setGlobalEnvVar(key, value) | Set a global environment variable and persist it to disk.     |
| bru.deleteGlobalEnvVar(key)     | Deletes a specific global environment variable and persists the change. |
| bru.deleteAllGlobalEnvVars()    | Deletes all global environment variables and persists the change. |
...
Sets an environment variable in the currently active environment and persists the change to disk. If the key already exists its value is silently overwritten. Accepts any value type — string, number, object.

```javascript
bru.setEnvVar("apiToken", "12345");

const data = res.getBody();
bru.setEnvVar("access_token", data.token);
bru.setEnvVar("user", { id: 42, role: "admin" });
```
...
Removes a specific variable from the current environment and persists the change to disk. Calling this on a key that does not exist is a silent no-op.
...
Sets or updates a global environment variable and persists the change to disk. If the key already exists its value is silently overwritten. Accepts any value type.
...
Removes a specific global environment variable and persists the change to disk. Calling this on a key that does not exist is a silent no-op.

```javascript
bru.deleteGlobalEnvVar("stale_token");
```

### `bru.deleteAllGlobalEnvVars()`

Removes all global environment variables and persists the change to disk. Calling this when no global variables exist is a silent no-op.

<Warning>
  This clears every global variable in the active global environment. It cannot be undone within the same run.
</Warning>
...
| bru.setCollectionVar(key, value)| Sets a collection variable and persists it to disk.                |
| bru.deleteCollectionVar(key)    | Deletes a specific collection variable and persists the change.    |
| bru.deleteAllCollectionVars()   | Deletes all collection variables and persists the change.          |
...
| bru.setVar(key, value)          | Sets a new runtime variable with a key-value pair.                 |
| bru.getAllVars()                | Retrieves all runtime variables as an object.                      |
| bru.deleteVar(key)              | Deletes a specific runtime variable.                               |
...
Sets a collection-level variable and persists the change to disk. If the key already exists its value is silently overwritten. Accepts any value type.

```javascript
bru.setCollectionVar("retryCount", 3);
bru.setCollectionVar("featureFlags", { darkMode: true });
```

### `bru.deleteCollectionVar(key)`

Removes a specific collection-level variable and persists the change to disk. Calling this on a key that does not exist is a silent no-op.

```javascript
bru.deleteCollectionVar("tempConfig");
```

### `bru.deleteAllCollectionVars()`

Removes all collection-level variables and persists the change to disk. Calling this when no collection variables exist is a silent no-op.

<Warning>
  This clears every variable in the collection scope. It cannot be undone within the same run.
</Warning>
...
const ca = fs.readFileSync(path.join(bru.cwd(), "certs", "ca.pem")); // collection root
...
Returns the absolute path of the collection's root directory on disk. Use this when a file lives at the collection root (certificates, shared fixtures, config). For a file next to the current request or folder `.yml` file, use [`__dirname`](#__dirname-and-__filename) instead.

Requires [Developer Mode](/get-started/configure/javascript-sandbox#developer-mode) to read the filesystem.
...
// Load a fixture from the collection root
const fs = require("fs");
const path = require("path");
const fixture = JSON.parse(fs.readFileSync(path.join(bru.cwd(), "fixtures", "user.json"), "utf8"));
req.setBody(fixture);
```

### `__dirname` and `__filename`

CommonJS globals for the **executing script's location** in the collection file tree. Available in both the Node VM sandbox and the isolated JS sandbox when the collection is in [Developer Mode](/get-started/configure/javascript-sandbox#developer-mode).

Use them to build paths next to the current `.yml` file:

```javascript
const fs = require("fs");
const path = require("path");

const csvData = fs.readFileSync(path.join(__dirname, "data.json"), "utf8");
```

| Script context | `__dirname` | `__filename` |
| -------------- | ----------- | ------------ |
| Collection (pre / post / tests) | Collection directory | Absolute path to `opencollection.yml` |
| Folder (pre / post / tests) | Folder directory | Absolute path to `folder.yml` |
| Request (pre / post / tests) | Request directory | Absolute path to the request `.yml` file |
| Fallback (no script path) | Collection directory | `undefined` |

If Bruno cannot determine the script path, `__dirname` falls back to the collection directory and `__filename` is `undefined`.

`bru.cwd()` is always the collection root. `__dirname` follows the folder that owns the script.
...
Executes another request in the collection by its path and returns its response. Useful for chaining dependent requests inside a single script without relying on collection runner order.

The argument is the request's path relative to the collection root, without the `.bru` or `.yml` extension. For a request at the top of the collection, pass just the request name (for example, `"Login"`). For a request inside a folder, include the folder path using forward slashes (for example, `"Auth/Login"`).

If Bruno cannot find a request at that path, the promise rejects with `bru.runRequest: invalid request path - <resolved path>`. Wrap the call in `try/catch` to handle a mistyped name.
...
<Info>
  `bru.runRequest()` only runs HTTP and GraphQL requests. WebSocket and gRPC requests resolve with `{ status: 'skipped' }` and are recorded in the Timeline as skipped rather than executed.
</Info>
...
```javascript
// Run an auth request at the top of the collection
const authResponse = await bru.runRequest("Login");
const token = authResponse.data.token;
req.setHeader("Authorization", "Bearer " + token);

// Run a request that lives inside a folder
const cleanupResponse = await bru.runRequest("Admin/Delete Test User");
...
gRPC request scripts use `bru.grpc.request` and `bru.grpc.response` instead of `req` / `res`. Open a gRPC request, go to the **Script (Beta)** tab, and write hooks there. Lifecycle, storage, and hook tables are on [gRPC Scripting](/send-requests/grpc/scripting).

`bru.cookies.*` and `bru.runner.*` are not available in gRPC hooks. Variables, `bru.sendRequest`, `bru.sleep`, `bru.interpolate`, and OAuth2 credential helpers are.

Hook abbreviations: **BCS** Before Call Start · **BMS** Before Message Send · **AMR** After Message Receive · **ACE** After Call End.

A message is `{ data, timestamp }`. `data` is the protobuf payload. `timestamp` is epoch milliseconds.

### `bru.grpc.request.messages`

Transmitted messages (not the list authored in the UI). Empty in BCS. Read-only in every hook.

```javascript
// ACE: inspect what went on the wire
const first = bru.grpc.request.messages.get(0);
console.log(first.data, first.timestamp);

const all = bru.grpc.request.messages.all();
const n = bru.grpc.request.messages.count();
const later = bru.grpc.request.messages.filter((m) => m.data.id > 1);
```

### `bru.grpc.request.message`

Current outbound message. **BMS only**, read-only.

```javascript
// BMS
console.log(bru.grpc.request.message.data.clientId);
console.log(bru.grpc.request.message.timestamp);
```

### `bru.grpc.response.message`

Current inbound message. **AMR only**.

```javascript
// AMR
const greeting = bru.grpc.response.message.data.greeting;
test("greeting is a string", function () {
  expect(greeting).to.be.a("string");
});
```

### `bru.grpc.response.messages`

All received messages. **ACE only**.

```javascript
// ACE
expect(bru.grpc.response.messages.count()).to.be.at.least(1);
const greetings = bru.grpc.response.messages.map((m) => m.data.greeting);
const last = bru.grpc.response.messages.get(bru.grpc.response.messages.count() - 1);
```

### `bru.grpc.request.metadata`

Client metadata. Readable in all hooks. Writable **only in BCS**. Keys are case-insensitive.

```javascript
// BCS — set metadata before the call opens
bru.grpc.request.metadata.upsert("authorization", "Bearer " + bru.getEnvVar("token"));
bru.grpc.request.metadata.add({ key: "x-app", value: "1" });
bru.grpc.request.metadata.remove("x-trace-id");

console.log(bru.grpc.request.metadata.get("authorization"));
console.log(bru.grpc.request.metadata.one("authorization"));
console.log(bru.grpc.request.metadata.toObject());
console.log(bru.grpc.request.metadata.has("x-app"));
console.log(bru.grpc.request.metadata.count());
```

### `bru.grpc.response.metadata`

Server initial metadata. **AMR and ACE**, read-only.

```javascript
// AMR or ACE
expect(bru.grpc.response.metadata.get("content-type")).to.equal("application/grpc");
console.log(bru.grpc.response.metadata.toObject());
```

### `bru.grpc.response.trailers`

Trailing metadata, including `grpc-status` and `grpc-message`. **ACE only**.

```javascript
// ACE
expect(bru.grpc.response.trailers.get("grpc-status")).to.equal("0");
expect(bru.grpc.response.trailers.has("grpc-status")).to.be.true;
console.log(bru.grpc.response.trailers.toObject());
```

### Call info

Available as listed. Status, status text, and duration are **ACE only**.

```javascript
console.log(bru.grpc.request.url);        // "localhost:50051"
console.log(bru.grpc.request.method);     // "/helloworld.Greeter/SayHello"
console.log(bru.grpc.request.methodType); // "unary" | "server-streaming" | ...
console.log(bru.grpc.request.authMode);   // "bearer" | "none" | ...
console.log(bru.grpc.request.protoPath);
console.log(bru.grpc.request.name);

// ACE
expect(bru.grpc.response.statusCode).to.equal(0);
expect(bru.grpc.response.statusText).to.equal("OK");
console.log(bru.grpc.response.duration);
```

### Tests and assertions

`test()` and `expect()` run in **AMR** (per message) and **ACE** (once at the end). Results show in the Tests tab.

```javascript
// AMR — one assertion per received message
test("payload has greeting", function () {
...
test("status is OK", function () {
  expect(bru.grpc.response.statusCode).to.equal(0);
  expect(bru.grpc.response.duration).to.be.a("number");
});
```

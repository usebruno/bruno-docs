...
Requires [Developer Mode](/get-started/configure/javascript-sandbox#developer-mode) to use the `fs` package to read the filesystem.
...
CommonJS globals for the **executing script's location** in the collection file tree. Available in both the Node VM sandbox and the isolated JS sandbox when in [Developer Mode](/get-started/configure/javascript-sandbox#developer-mode).
...
gRPC scripts use `bru.grpc.request` and `bru.grpc.response` instead of `req` / `res`. Open a gRPC request, go to the **Script (Beta)** tab, and write hooks there. Lifecycle, storage, and hook tables are on [gRPC Scripting](/send-requests/grpc/scripting).

`bru.cookies.*` and `bru.runner.*` are not available in gRPC scripts. Variables, `bru.sendRequest`, `bru.sleep`, `bru.interpolate`, and OAuth2 credential helpers are.

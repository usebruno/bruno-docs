...
gRPC request scripts use `bru.grpc.request` and `bru.grpc.response` instead of `req` / `res`. Enable **gRPC Scripting** under **Preferences → Beta**. Lifecycle, storage, and hook tables are on [gRPC Scripting](/send-requests/grpc/scripting).

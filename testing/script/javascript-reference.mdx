...
Sets a variable in the currently selected environment. Changes are written back to the environment file on disk by default, so values set by scripts survive the request and persist across app restarts.
...
<Info>
  The legacy third argument `{ persist: true }` is no longer required — all scripted changes persist by default. Calls that still pass it continue to work for backward compatibility.
</Info>
...
Removes a specific variable from the current environment. Deletions are written back to the environment file on disk so they persist across requests and app restarts.
...
<Warning>
  This action clears all variables from the active environment and writes the change to disk. It cannot be undone within the same run.
</Warning>
...
Removes a specific variable from the active global environment. Deletions are written back to disk so they persist across requests and app restarts.

```javascript
// Drop a stale global token after rotating credentials
bru.deleteGlobalEnvVar("global_token");
```

### `bru.deleteAllGlobalEnvVars()`

Removes every variable from the active global environment.

<Warning>
  This action clears all variables from the active global environment and writes the change to disk. It cannot be undone within the same run.
</Warning>
...
| bru.setCollectionVar(key, value) | Sets a collection-level variable and persists it to disk. |
| bru.deleteCollectionVar(key) | Deletes a specific collection-level variable.             |
| bru.deleteAllCollectionVars() | Deletes all collection-level variables.                  |
| bru.getAllCollectionVars()   | Retrieves all collection-level variables as an object.    |
...
Sets a collection-level variable. Changes are written back to the collection file (`collection.bru` / `collection.yml`) on disk so values set by scripts survive the request and persist across app restarts.

```javascript
// Store a base URL discovered at runtime so every subsequent request can reach it
bru.setCollectionVar("baseUrl", "https://api.staging.example.com");

// Cache a refresh token at the collection scope
const data = res.getBody();
bru.setCollectionVar("refresh_token", data.refresh_token);
```

### `bru.deleteCollectionVar(key)`

Removes a specific variable from the collection. Deletions are written back to the collection file on disk.

```javascript
bru.deleteCollectionVar("refresh_token");
```

### `bru.deleteAllCollectionVars()`

Removes every collection-level variable.

<Warning>
  This action clears all variables from the collection and writes the change to disk. It cannot be undone within the same run.
</Warning>
...
Returns an object containing all collection-level variables as key-value pairs.

```javascript
const collectionVars = bru.getAllCollectionVars();
console.log("Collection variables:", collectionVars);
// Output: { baseUrl: "https://api.example.com", namespace: "production" }
```

...
| Method | Description |
| ------ | ----------- |
| `bru.cookies.each(fn)` | Invoke `fn` for each cookie (e.g. `(c) => { ... }` with `c.key` / `c.value`). |
| `bru.cookies.find(fn)` | First cookie matching the predicate. |
| `bru.cookies.filter(fn)` | All cookies matching the predicate. |
| `bru.cookies.map(fn)` | Map each cookie to a new value. |
| `bru.cookies.reduce(fn, initial)` | Reduce over cookies. |

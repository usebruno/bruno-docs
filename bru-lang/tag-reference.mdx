...
Use `@contentType(<mime>)` before a field to set the `Content-Type` of that individual multipart part. This is preserved on save, export, and import; a blank or whitespace-only value is not serialized.
## body:graphql

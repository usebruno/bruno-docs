---
title: "Bru Tag Reference"
sidebarTitle: "Tag Reference"
---

## meta
Store metadata about your request
```bash
meta {
  name: Get users
  type: http
  seq: 1
  tags: [
    smoke
    sanity
  ]
}
```

The `seq` is used to store the sequence number. This decides the sort position of your request in the UI.
The `type` can be either `http` or `graphql`
The `tags` is an array of strings that can be used to filter requests during collection runs. Tags are useful for organizing and selectively running requests based on categories like environment, functionality, or priority.

Folders support the same `tags` field in their `folder.bru` root file. Folder tags cascade to every request and subfolder inside that folder, and a request matches on the union of its own tags and any inherited tags.

```bash
meta {
  name: users
  seq: 2
  tags: [
    smoke
  ]
}
```

## get

Make a `GET` http call
```bash
get {
  url: https://api.github.com/users/usebruno
}
```
## post

Make a `POST` http call
```bash
post {
  url: https://api.github.com/users/usebruno
}
```
## put

Make a `PUT` http call
```bash
put {
  url: https://api.github.com/users/usebruno
}
```
## delete

Make a `DELETE` http call
```bash
delete {
  url: https://api.github.com/users/usebruno
}
```
## options

Make a get `OPTIONS` call
```bash
options {
  url: https://api.github.com/users/usebruno
}
```
## trace

Make a `TRACE` http call
```bash
trace {
  url: https://api.github.com/users/usebruno
}
```
## connect

Make a `CONNECT` http call
```bash
connect {
  url: https://api.github.com/users/usebruno
}
```
## head

Make a `HEAD` http call
```bash
head {
  url: https://api.github.com/users/usebruno
}
```
## query

The request query params
```bash
get {
  url: https://api.textlocal.in/send?apiKey=secret&numbers=9988776655&message=hello
}

params:query {
  @description('''Your API access key''')
  apiKey: secret
  @description('''Comma-separated list of recipient numbers''')
  numbers: 9988776655
  message: hello
}
```
## path

The request path params
```bash
get {
  url: https://api.textlocal.in/user/:userId
}

params:path {
  userId: 13
}
```
## headers

The request headers
```bash
get {
  url: https://api.textlocal.in/send?apiKey=secret&numbers=9988776655&message=hello
}

headers {
  content-type: application/json
  @description('''Bearer token obtained after login. Expires in 24 hours.''')
  Authorization: Bearer topsecret
}
```

## body
The request body (defaults to json)
```bash
body {
  {
    username: 'john',
    password: 'governingdynamics'
  }
}
```
## body:text
The request body as text
```bash
body:text {
  This is a text body
}
```
## body:xml
The request body as xml
```bash
body:xml {
  <xml>
    <name>John</name>
    <age>30</age>
  </xml>
}
```
## body:form-urlencoded
The request body as form-urlencoded
```bash
body:form-urlencoded {
  @description('''Your API access key''')
  apikey: secret
  @description('''Recipient phone number with country code''')
  numbers: +91998877665
  ~message: hello
}
```

Use `@description` before a field to document what it represents. Descriptions are optional.
## body:multipart-form
The request body as multipart-form
```bash
body:multipart-form {
  @description('''Your API access key''')
  apikey: secret
  @description('''Recipient phone number with country code''')
  numbers: +91998877665
  ~message: hello
}
```

Use `@description` before a field to document what it represents. Descriptions are optional.
## body:graphql
The request body as graphql
```bash
body:graphql {
  {
    launchesPast {
      launch_site {
        site_name
      }
      launch_success
    }
  }
}
```
## body:graphql:vars
The request body as graphql vars
```bash
body:graphql:vars {
  {
    "limit": 5
  }
}
```
## script:pre-request
The request body as pre-request
```bash
script:pre-request {
  req.setHeader("Authorization", "{{token}}");
}
```
## script:post-response
The request body as post-response
```bash
script:post-response {
  bru.setVar("token", res.body.token);
}
```
## test
The tests
```bash
body:test {
  expect(res.status).to.equal(200);
}
```
